...
{
	UE_LOG(LogTemp, Log, TEXT("Player [%s] alive status changed to: %s"), *GetPlayerName(), bAlive ? TEXT("Alive") : TEXT("Dead"));
}

void ATKPlayerStateBase::OnRep_Identity()
{
	UE_LOG(LogTemp, Log, TEXT("Player [%s] identity revealed: %d"), *GetPlayerName(), (uint8)Identity);
}

void ATKPlayerStateBase::OnRep_Health()
{
	UE_LOG(LogTemp, Log, TEXT("Player [%s] health: %d/%d"), *GetPlayerName(), CurrentHealth, MaxHealth);
}
...
bool ATKPlayerStateBase::ApplyDamage(int32 Damage)
{
	...
	UE_LOG(LogTemp, Log, TEXT("Player [%s] took %d damage, health now: %d"), *GetPlayerName(), Damage, CurrentHealth);

	if (CurrentHealth <= 0)
	{
		bAlive = false;
		UE_LOG(LogTemp, Log, TEXT("Player [%s] has died!"), *GetPlayerName());
		return true; // died
	}
	return false;
}
...
bool ATKPlayerStateBase::Heal(int32 HealAmount)
{
	...
	UE_LOG(LogTemp, Log, TEXT("Player [%s] healed %d, health now: %d"), *GetPlayerName(), HealAmount, CurrentHealth);
	return true;
}
